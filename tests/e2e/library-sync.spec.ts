...
test('library manual sync should work', async () => {
    ...
    try {
        ...
        const syncButton = window.locator('button:has-text("Sync Library"), button:has-text("ライブラリの同期")')
        await expect(syncButton).toBeVisible()

        const rescanButton = window.locator('button:has-text("Force Rescan"), button:has-text("強制再読み込み")')
        ...
        await window.click('button:has(svg.lucide-x), button:has-text("Cancel"), button:has-text("キャンセル")')
