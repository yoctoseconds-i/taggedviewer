...
export async function startWorkerMode() {
  ...
  if (!port.on) {
    console.error(
      '[Worker] No communication port found. This script should be run as a utilityProcess or fork.'
    )
    process.exit(1)
  }
        ...
        response.on('data', (chunk) => {
          ...
          if (totalSize > 0) {
            ...
            if (percent % 10 === 0 && percent !== lastLoggedPercent) {
              console.log(
                `[Worker] Downloading... ${percent}% (${(downloadedSize / 1024 / 1024).toFixed(1)} MB / ${(totalSize / 1024 / 1024).toFixed(1)} MB)`
              )
    ...
    if (type === 'TAG_IMAGE') {
      try {
        ...
        for (let i = 0; i < size * size; i++) {
          floatData[i * 3] = data[i * 4 + 2]
          floatData[i * 3 + 1] = data[i * 4 + 1]
          floatData[i * 3 + 2] = data[i * 4]
        }
        console.log(`[Worker] Running inference...`)
        const results = await sess.run({
          [sess.inputNames[0]]: new onnx.Tensor('float32', floatData, [1, size, size, 3]),
        })
        const output = results[sess.outputNames[0]].data as Float32Array
        const finalTags = []
        for (let j = 0; j < output.length; j++)
          if (output[j] > 0.35 && tags[j]) finalTags.push(tags[j])
